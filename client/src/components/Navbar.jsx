...
const Navbar = () => {
  ...
  return (
    <nav className="bg-gradient-to-l from-gray-900 to-gray-600 text-white shadow-md p-4 fixed top-0 left-0 w-full z-50">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-gray-700  p-4 rounded-lg">
          ...
          {loading ? (
            ...
            <button
              onClick={() => handleMenuItemClick("/profile")}
              className="block w-full text-left px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-400"
            >
              <FaUser className="mr-2" /> {user?.displayName || "Profile"}
            </button>
          ) : (
            <button
              onClick={() => navigate("/login")}
              className="block w-full text-left px-4 py-2 flex items-center text-white rounded-lg hover:bg-blue-400"
            >
              <FaSignInAlt className="mr-2" /> Login
            </button>
          )}
